...
for image in images:
    ...
    if "okay" in quality or "good" in quality:
        ...
        type_qual="wrong"
    y_train.append([quality,type_qual])
    img_det = [image_name,type_qual]
    index+=1
...
pipe=Pipeline(steps=[('classifier',obj),('gamma',FixedGammaCorrection()),('pillcount',Yolov4Classifier())]) 

gamma_params = config['gamma_params']
param_grid={**config['classifier_params'],**gamma_params}
